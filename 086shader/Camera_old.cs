...
namespace _086shader
{
  ...
  {
    private bool fileScript = false;
    private List<Tuple<double,Vector3,Vector3>> scriptSteps;
    /// <summary>
    ...
    {
      if (cameraFile != "" && cameraFile != null && File.Exists(cameraFile))
        fileScript = true;
    ...
    private void processParametersFile (string param, string cameraFile)
    {
      string[] lines = File.ReadAllLines(cameraFile);
    ...
    Matrix4 computeModelView ()
    {
      ...
      Vector3 eye = Center + new Vector3((float)(Math.Sin(t) * r),
                                         ...
                                         (float)(Math.Cos(t) * r) );
      return Matrix4.LookAt(eye, Center, Vector3.UnitZ);
    }
