...
namespace _087fireworks
{
  ...
  public class Launcher : DefaultRenderObject
  {
    ...
    /// </summary>
    public Vector3d position,position0,velocity;
    ...
    /// </summary>
    public double simTime,simTime0;
    ...
    public void ExplodeMoreLaunchers(double time, Fireworks fw)
    {
      int splits = rnd.RandomInteger(5,10);
      for(int i = 0; i < splits; i++)
      {
        Launcher l;
        Vector3d dir = Geometry.RandomDirectionNormal(rnd, aim, 10);         // random direction around 'aim'
        if(rnd.RandomInteger(1,2) == 1)
          l = new Launcher(40, time, rnd.RandomDouble(2.0, 12.0), true, position, dir, new Vector3d(0, 1, 0));
        else
          l = new Launcher(40, 0, rnd.RandomDouble(2.0, 12.0), true, new Vector3d(0, 0.0, 0.0), dir, new Vector3d(0, 1, 0));
        fw.AddLauncher(l);
      }
    }

    public void Explode(double time, Fireworks fw)
    {
      bool dup = false;
      if(rnd.RandomInteger(1,5) == 1)
      {
        ExplodeMoreLaunchers(time,fw);
        return;
      }
      if (rnd.RandomInteger(1, 10) == 1)
        dup = true;
      for(int i = 0; i < Math.Max(1000,frequency);i++)
      {
        Vector3d dir = Geometry.RandomDirectionNormal(rnd, aim, 1000);         // random direction around 'aim'
        fw.AddParticle(new Particle(position, dir * rnd.RandomDouble(5,8), up,
          new Vector3(rnd.RandomFloat(0.1f, 1.0f), rnd.RandomFloat(0.1f, 1.0f), rnd.RandomFloat(0.1f, 1.0f)), rnd.RandomDouble(0.2, 4.0), time, rnd.RandomDouble(2.0, 12.0),dup));
      }
    }
    ...
    public bool Simulate (double time, Fireworks fw)
    {
             ...
             rnd.UniformNumber() < probability)
      {
        ...
        Particle p = new Particle(position, dir * rnd.RandomDouble(0.2, 0.8), up,
                                  ...
                                  rnd.RandomDouble(0.2, 4.0), time, rnd.RandomDouble(2.0, 12.0),false);
        fw.AddParticle(p);
        probability -= 1.0;
      }
      ...
      if (going)
      {
        double t = time - simTime0;
        position.X = position0.X + velocity.X * t;
        position.Y = position0.Y + velocity.Y * t - 0.5 * t * t * (GRAVACC / 20.0);
        position.Z = position0.Z + velocity.Z * t;

        //position = position + aim / 50.0;
      }
      return true;
    }
    private const double GRAVACC = 9.8067; //mm/sec^2

    public Launcher (double freq, double _time, double _age, bool _going, Vector3d? pos = null, Vector3d? _aim = null, Vector3d? _up = null)
    {
      ...
      going = _going;
      position0 = position;
      velocity = aim * rnd.RandomInteger(5,10);
    }
  ...
  public class Particle : DefaultRenderObject
  {
    ...
    /// </summary>
    public Vector3d velocity,velocity0;
    ...
    public double simTime0;
    private const double GRAVACC = 9.8067; //mm/sec^2

    private bool duplicate;

    public Particle (Vector3d pos, Vector3d vel, Vector3d _up, Vector3 col, double siz, double time, double age, bool _duplicate)
    {
      position = pos;
      position0 = pos;
      velocity = vel;
    ...
    public void Explode (double time, Fireworks fw)
    {
      ...
      for (int i = 0; i < 100; i++)
      {
        Vector3d dir = Geometry.RandomDirectionNormal(rnd, velocity, 1000);         // random direction around 'aim'
        fw.AddParticle(new Particle(position, dir * rnd.RandomDouble(0.2, 1.8), up,
          new Vector3(rnd.RandomFloat(0.1f, 1.0f), rnd.RandomFloat(0.1f, 1.0f), rnd.RandomFloat(0.1f, 1.0f)), rnd.RandomDouble(0.2, 4.0), time, rnd.RandomDouble(2.0, 12.0),false));
      }
    }
    ...
    public bool Simulate (double time, Fireworks fw)
    {
      ...
      position.X = position0.X + velocity.X * t;
      position.Y = position0.Y + velocity0.Y * t - 0.5 * t * t * (GRAVACC/20.0);
      position.Z = position0.Z + velocity.Z * t;
  ...
  public class Fireworks
  {
    ...
    public void Reset (string param)
    {
      ...
      for (int i = 0; i < k; i++)
      {
        double angle = rnd.RandomDouble(60.0,120.0);
        double rad = Math.PI * angle /180.0;
        double x = Math.Cos(rad);
        double y = Math.Sin(rad);
        double z = Math.Cos(rad * r);
        double rx = rnd.RandomDouble(-10,10);
        double ry = rnd.RandomDouble(-10,10);
        double rz = rnd.RandomDouble(-10,10);

        l = new Launcher(40,0, rnd.RandomDouble(2.0, 12.0),true, new Vector3d(rx, ry, rz), new Vector3d(x, y, z), new Vector3d(0,1, 0));
        AddLauncher(l);
    ...
    public void Simulate (double time)
    {
      ...
      { 
        for (i = 0; i < launchers.Count; i++)
          if (!launchers[i].Simulate(time, this))
          {
            expiredLaunchers.Add(i);
          }
      }
      else
      {
      ...
      expiredLaunchers.Sort(ReverseComparer);
      foreach (int j in expiredLaunchers)
        launchers.RemoveAt(j);
      ...
      else if(rnd.RandomDouble(1,100) <= prob)
      {
        Launcher l;
        double angle = rnd.RandomDouble(60.0,120.0);
        double rad = Math.PI * angle /180.0;
        ...
        double y = Math.Sin(rad);
        double z = Math.Cos(rad * r);
        l = new Launcher(40, time, rnd.RandomDouble(2.0, 12.0), true, new Vector3d(rnd.RandomDouble(-10, 10), rnd.RandomDouble(-10, 10), rnd.RandomDouble(-10, 10)), new Vector3d(x, y, z), new Vector3d(0, 1, 0));
        AddLauncher(l);
    ...
    void addLaunchers(double time)
    {
      Launcher l;
      for (int i = 0; i < k; i++)
      {
        double angle = rnd.RandomDouble(0.0,180.0);
        double rad = Math.PI * angle /180.0;
        double x = Math.Cos(rad);
        double y = Math.Sin(rad);
        double z = Math.Cos(rad * r);
  ...
  public partial class Form1
  {
    ...
    {
      param           = "freq=400.0,max=60000,slow=0.25,dynamic=1,variance=0.1,ticks=0,prob=0,k=4";
      tooltip         = "freq,max,slow,dynamic,variance,ticks,screencast,prob,k";
